...
func (c *DnsController) handle_(
    ...
    // Route request.
    upstreamIndex, rewrite, upstream, err := c.routing.RequestSelect(qname, qtype)
    if err != nil {
        return err
    }
    ...
    if upstreamIndex == consts.DnsRequestOutboundIndex_Rewrite {
        rr := make([]dnsmessage.RR, 0)
        ip4 := rewrite.To4()
        var answer dnsmessage.RR
        ...
        } else if qtype == dnsmessage.TypeAAAA {
            ip6 := rewrite.To16()
            if ip4 == nil {
                answer = &dnsmessage.AAAA{
                    Hdr: dnsmessage.RR_Header{
                        Name:   dnsmessage.CanonicalName(qname),
                        Class:  dnsmessage.ClassINET,
                        Ttl:    0,
        ...
        if answer != nil {
            ...
            if needResp {
                if err = sendPkt(c.log, data, req.realDst, req.realSrc, req.src, req.lConn); err != nil {
                    return fmt.Errorf("failed to write rewritten DNS resp: %w", err)
                }
            }
            if c.log.IsLevelEnabled(logrus.DebugLevel) && len(dnsMessage.Question) > 0 {
                q := dnsMessage.Question[0]
                c.log.Debugf("UDP(DNS) %v <-> Rewrite: %v %v -> %v",
                    RefineSourceToShow(req.realSrc, req.realDst.Addr()), strings.ToLower(q.Name), QtypeToString(q.Qtype), answer.String(),
                )
            }
            return nil
        }
